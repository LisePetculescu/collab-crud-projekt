...
async function start() {
  ...
  document.querySelector("#btn-yes-update").addEventListener("click", blurRemoved);
  document.querySelector("#btn-yes-delete").addEventListener("click", blurRemoved);
  document.querySelector("#btn-no-update").addEventListener("click", blurRemoved);
  document.querySelector("#btn-no-delete").addEventListener("click", blurRemoved);
...
function showCharacterModal(character) {
  ...
  const html = /* HTML */ `
    <article class="modal-item">
      <h3>${character.name} <button id="btn-close" class="buttonAni">Back</button></h3>
      <img src="${character.image}" />
      <p>Race: ${character.race}</p>
      <p>Age: ${age}</p>
      <p>Actor: ${character.actor}</p>
